...
- [ChatMessageStore.cs](/d:/projects/super-chat/src/SuperChat.Infrastructure/Features/Messaging/ChatMessageStore.cs)
...
3. Мы сохраняем его в `chat_messages`.
...
Рекомендуемые варианты старта:

- `BGE-M3`, если есть отдельная машина или лёгкий внешний sidecar
- `Yandex AI Studio text-search-doc / text-search-query`, если нужен внешний embeddings API без тяжёлой модели на VPS
...
Сейчас их роль у нас уже выполняет `chat_messages`.
...
- `chat_messages` = наши текущие `message_events`
...
- появился `EmbeddingOptions` и .NET-клиент с выбором backend
- `LocalService` по-прежнему работает через `infra/embedding-service/`
- `YandexCloud` позволяет ходить за dense embeddings напрямую в Yandex AI Studio без обязательного sidecar-контейнера
- по умолчанию локальный сервис работает в безопасном `mock`-режиме, чтобы stage 3 можно было поднимать без тяжёлой ML-модели
...
- `RetrievalService` считает query embeddings через настроенный embedding backend
...
- перегружать смысл существующего `Processed` в `chat_messages`
...
- считать текущие `chat_messages` каноническим сырьём
...
- stage 3 embeddings уже реализованы как абстракция с двумя backend-режимами: optional Python sidecar и прямой `YandexCloud`
