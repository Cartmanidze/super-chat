...
public sealed class WorkItemWriterTests
{
    [Fact]
    public async Task AcceptRangeAsync_DeduplicatesBatchByUserSourceEventIdAndKind_KeepingHighestConfidence()
    {
        ...
        var service = new WorkItemWriter(
            factory,
    ...
    [Fact]
    public async Task AcceptRangeAsync_SkipsExistingWorkItemWithSameUserSourceEventIdAndKind()
    {
        var factory = await CreateFactoryAsync(CancellationToken.None);
        var userId = Guid.NewGuid();
        var observedAt = new DateTimeOffset(2026, 04, 07, 09, 00, 00, TimeSpan.Zero);
    ...
    private static async Task<IDbContextFactory<SuperChatDbContext>> CreateFactoryAsync(CancellationToken cancellationToken)
    {
        var dbContextOptions = new DbContextOptionsBuilder<SuperChatDbContext>()
            .UseInMemoryDatabase($"superchat-work-item-writer-{Guid.NewGuid():N}")
            .Options;
